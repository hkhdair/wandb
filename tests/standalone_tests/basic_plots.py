...
def main(argv):
    ...
    xs = [
        list(range(100)),
        list(range(100)),
        [2 * j for j in range(50)],
        [2 * j for j in range(50)],
    ]
    ys = [
        [random.random() + math.sin(i / 10) for i in range(100)],
        [math.sin(i / 10) for i in range(100)],
        [math.cos(i / 10) for i in range(50)],
        [random.random() - math.cos(i / 10) for i in range(50)],
    ]
    ...
    line_series_singular_x_array = wandb.plot.line_series(
        list(range(100)), ys, title="Get serial with one x", xname="step"
    )
